...
`npx agentfloor` — 어떤 프로젝트에서든 에이전트를 허브에 등록하고 관리하는 CLI 도구.
...
```
$ npx agentfloor login
? AgentFloor Hub URL [http://localhost:3000]: https://agentfloor.vercel.app
✓ Hub connected.
...
4. 글로벌 config 저장 (`~/.agentfloor/config.json`)
...
ℹ Pushing to "Acme Corp" at https://agentfloor.vercel.app...
✓ Agent registered! (agent-17345678)

Dashboard: https://agentfloor.vercel.app
```
...
5. 프로젝트 config 저장 (`.agentfloor/config.json`)

**업데이트:**
- 이미 등록된 에이전트가 있으면 (`.agentfloor/config.json`에 `agentId` 존재) 자동으로 업데이트
...
Agent Status
  ...
  Hub URL:       https://agentfloor.vercel.app
...
Run `agentfloor push` to register.
```
...
### 글로벌 config: `~/.agentfloor/config.json`
...
```json
{
  "organizations": [
    {
      "hubUrl": "https://agentfloor.vercel.app",
      "orgId": "org-12345",
      "orgName": "Acme Corp",
      "inviteCode": "C2M2XF",
      "memberName": "Alice"
...
### 프로젝트 config: `.agentfloor/config.json`
...
**`Not logged in`**
- `agentfloor login`을 먼저 실행
- `~/.agentfloor/config.json` 파일이 존재하는지 확인
